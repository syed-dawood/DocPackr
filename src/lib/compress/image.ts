...
export async function compressImageBlob(input: File | Blob): Promise<ImageCompressResult> {
  ...
  const file = input instanceof File ? input : new File([input], 'image', { type: input.type || 'image/jpeg' })
  ...
  const out: Blob = compressed instanceof File ? compressed : new Blob([compressed], { type: compressed.type || file.type })
